...
def read_from_stream():
	...
	df_val = df.withColumn("title", udf_json(df.ACTUAL_VALUE))
	start = time.time()
	df_sent = df_val.withColumn("_sentiment", udf_analyzer_function(df_val.title))
	end = time.time()
	
	df_eps = spark.createDataFrame([(f"{end-start}",)], schema=["Elapsed time!!!!",])

	
	df_wc = df_sent.withColumn("_wc", udf_naive_wc(df_val.title))	

	df_wc.select("title", "_sentiment", "_wc").writeStream.format("console").start().awaitTermination()
